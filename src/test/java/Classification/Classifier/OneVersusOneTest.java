...
public class OneVersusOneTest extends ClassifierTest {
    ...
    public void testTrain() throws DiscreteFeaturesNotAllowed {
        ...
        assertEquals(4.666666666666663, 100 * oneVersusOne.test(iris.getInstanceList()).getErrorRate(), 0.01);
        ...
        assertEquals(3.825136612021862, 100 * oneVersusOne.test(dermatology.getInstanceList()).getErrorRate(), 0.01);
        models.clear();
        for (int i = 0; i < (classLabelSize * (classLabelSize - 1)) / 2; i++) {
            models.add(new DecisionTree());
        }
        oneVersusOne.train(dermatology.getInstanceList(), c45Parameter);
        assertEquals(1.912568306010931, 100 * oneVersusOne.test(dermatology.getInstanceList()).getErrorRate(), 0.01);
    }
}
